...
\begin{defn}
  Let $a,b \in \mathbb{R}$. If $z \in \mathbb{C}$ is such that $z = a + bi$, we say
  that $a$ is the \textbf{real part} of $z$ and $b$ is the \textbf{imaginary part} of
  $z$. We may also write
  \begin{align*}
...
\begin{defn}
  Let $a,b \in \mathbb{R}$. If $z \in \mathbb{C}$ is such that $z = a + bi$, we say
  that the \textbf{conjugate} of $z$ is $a - bi$, and we represent this number by
  $\bm{\overline{z}}$.
\end{defn}
...
\begin{thm}
  ...
  \begin{enumerate}
    ...
    \item
      $z + \overline{z} = 2\operatorname{Re}(z)$ and $z - \overline{z} =
      2i\operatorname{Im}(z)$, and
    \item
      $z\overline{z}$ is both real and positive when $z \neq 0$.
  \end{enumerate}
  ...
  \begin{proof}
    ...
    \begin{enumerate}
      ...
      \item
        ...
        \end{align*}
      \item
        $z + \overline{z} = a + bi + a - bi = 2a = 2\operatorname{Re}(z)$. $z -
        \overline{z} = a + bi - (a - bi) = a + bi - a + bi = 2bi =
        2i\operatorname{Im}(z)$.
      \item
        If $z = 0$, then $z\overline{z} = 0$. Otherwise, $z\overline{z} = [a(a) -
        b(-b)] + [a(-b) + b(a)]i = (a^2 + b^2) + 0i = a^2 + b^2$. If $r$ is a real
        number such that $r \neq 0$, then $r^2 > 0$, so $a^2 + b^2 = z\overline{z} >
        0$.
    \end{enumerate}
  \end{proof}
\end{thm}
...
\begin{defn}
  Let $z \in \mathbb{C},\ a,b \in \mathbb{R}$ such that $z = a + bi$. We say that the
  \textbf{absolute value} of $z$ is the non-negative square root of $z\overline{z}$,
  i.e.  $(z\overline{z})^{1/2} = \sqrt{a^2 + b^2}$, and we represent it by $\bm{|z|}$.
\end{defn}

\begin{comm}
  If $z$ is real, i.e. $z = a + 0i$ for some $a \in \mathbb{R}$, then $\overline{z} =
  z$, so $|z| = \sqrt{z^2}$. In other words, our earlier definition of absolute value
  for real numbers is preserved under our definition for complex numbers.
\end{comm}
